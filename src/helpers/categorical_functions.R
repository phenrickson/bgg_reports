...
# preprocess
# function to pivot categorical features into dummies
pivot_categorical_features = function(games_categorical,
                                      min_games) {

        games_categorical %>%
                tidy_categorical_variables() %>%
                select(id, type, value, has_value, game_id) %>%
                group_by(id, type, value) %>%
                ...
                pivot_wider(names_from = value,
                            ...
                            id_cols = c("game_id"),
                            values_fill = list(has_value = 0))

}

pivot_categorical_features(game_designers,
                           min_games = 10)
# 
# # minimum games
# min_games = 10
...
#         tidy_categorical_variables() %>%
#         group_by(type, id, value, abbrev, level) %>%
#         mutate(n_games = n_distinct(game_id)) %>%
#         ungroup() %>%
#         filter(n_games > 10) %>%
#         pivot_wider(names_from = value, 
...
#         tidy_categorical_variables()
# 
# tidy_categorical_variables(game_mechanics)
# game_mechanics %>%
#         filter(grepl("Campaign", value))
