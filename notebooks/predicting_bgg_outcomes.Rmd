...
publisher_list = c(51,
            ...
            #       512,
                   ...
                   538,
                   52,
              #     8923, drop for now, seems to be also an outcome rather than the initial publisher
                   17,
                   5,
                   3320,
                   597,
...
average_tuned <- 
        average_set %>% 
        option_add(grid = glmnet_grid, id = "normalize_all_glmnet") %>%
        option_add(grid = glmnet_grid, id = "normalize_no_artistdesigner_glmnet") %>%
        option_add(grid = glmnet_grid, id = "normalize_no_publisher_glmnet") %>%
        option_add(grid = glmnet_grid, id = "normalize_no_categorical_glmnet") %>%
        option_add(grid = cart_grid, id = "full_cart") %>%
        option_add(grid = xgbTree_grid, id = "full_xgbTree") %>%
...
Wargames and simulation games tend to dominate the highest BGG averages; these games tend to have a lower number of ratings but a high average due to what I would guess is a pretty passionate fanbase.
...
Man, I would really like a second eye to review some of these. 
...
bgg_outcomes_final_workflows = bgg_outcomes_last_fits %>%
        ...
        select(.id, wflow_id, best_tune, last_fit) %>%
        mutate(.workflow = map2(.x = last_fit,
                               .y = best_tune,
                              ~ .x %>% 
                                      extract_workflow(.) %>%
                                      finalize_workflow(., .y) %>%
                                      fit(bind_rows(games_train,
                                                    games_validation)))) %>%
        select(.id, wflow_id, best_tune, .workflow) %>%
        left_join(., bgg_outcomes_last_fits %>% 
                          filter(grepl("xgbTree", wflow_id)) %>% 
                          unnest() %>% 
                          select(.id, wflow_id, splits),
                  by = c(".id", "wflow_id")) %>%
        mutate(outcome = gsub("_tuned", "", .id))
...
## Highest Expected Average

What upcoming games are expected to be have the highest average rating?
...
preds_test_tidied %>%
        ...
        left_join(., games_test %>%
                  ...
                  by = c("game_id", "name")) %>%
        filter(bayesaverage > 0) %>%
     #   rename(pred_xgbTree = bayesaverage_xgbTree) %>%
        rename(pred_stan = bayesaverage_indirect_stan,
               pred_xgbTree_direct = bayesaverage_xgbTree,
               pred_xgbTree_indirect = bayesaverage_indirect_xgbTree) %>%
        arrange(desc(pred_xgbTree_direct)) %>%
        mutate_if(is.numeric, round, 2) %>%
       # select(game_id, name, yearpublished, usersrated, pred_stan, pred_xgbTree) %>%
        select(game_id, name, yearpublished, bayesaverage, pred_stan, pred_xgbTree_direct, pred_xgbTree_indirect) %>%
        DT::datatable()
...
The model estimated the averageweight of the game to be lower than the BGG community found it, and part of the reason for this is because it is educational and has animals. A lower estimated averageweight then leads to a lower average rating. Similarly, looking at the estimates for usersrated,, the model lowered its estimate for Wingspan due to the combination of dice rolling and open drafting mechanics. Based on the data it had seen at the time, I'm not surprised that the model didn't think this would be a smash hit - nothing quite like Wingspan had ever come before it.
