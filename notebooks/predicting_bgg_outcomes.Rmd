...
To answer these questions, we'll make use of historical data from boardgamegeek. We will connect to a database containing game features and their current ratings on BGG. For this analysis, in training models, we will restrict ourserlves to games published through `r params$end_training_year`. We will validate the performance of our models by evaluating their performance in predicting games published in `r params$end_training_year + 1`.
...
```{r correlation between predictors and categories, fig.height=12, fig.width=12, fig.ca = "Correlation plot of game info and game categories"}
...
```{r mechanics density plots, fig.height=12, fig.width=10}

# get correlation with average
# geom density plot

# make a density plot for mechanics and outcomes
mechanics_density = df %>%
        select(average,
               bayesaverage,
               usersrated,
               averageweight,
        ...
        gather("feature", "feature_value",
               ...
               -usersrated,
               -averageweight) %>%
        mutate(feature = tidy_name_func(feature)) %>%
        gather("outcome", "outcome_value",
               -feature,
               -feature_value) %>%
        mutate(feature_value = case_when(feature_value == 1 ~ 'yes',
                                         TRUE ~ 'no')) %>%
        filter(feature_value == 'yes') %>%
        group_by(feature, outcome) %>%
        mutate(median_outcome_value = median(outcome_value)) %>%
        ungroup() %>%
        group_by(outcome) %>%
        mutate(median = median(outcome_value)) %>%
        ungroup() %>%
        ggplot(., aes(x=outcome_value,
                     # fill = feature_value,
                      y = reorder_within(feature, median_outcome_value, outcome)))+
        stat_density_ridges(quantile_lines = TRUE, quantiles = 0.5, alpha = 0.8)+
        facet_wrap(outcome~.,
                   ncol = 4,
                   scales="free")+
        theme_bw(8)+
...
We'll register a parallel back end and then train models for each outcome.

```{r register parallel back end}
...
```{r tune models with tuning parameters, eval=T} 
...
## Assessing Models via the Validation Set
...
Generally speaking, the boosted trees tend to exhibit the strongest performance regardless of outcome and evaluation metric, though the improvement over the linear models is not dramatic. If we plot the predictions from the models for a sample of games, we can get a sense of where they agree and where they consistently miss.

```{r evaluate, warning=F, message=F, fig.height=10, fig.width=8}
...
```{r pull draws and compare, fig.height=10, fig.width=8, warning=F, message=F}
...
```{r refit models to training and validation}
...
final_models = ls(pattern="final_fit")

for(i in 1:length(final_models)) {
        
        readr::write_rds(
                rlang::sym(final_models[i]),
                 paste(here::here("models", "repository",
                                  final_models[i[]]),
                       "_", Sys.Date(), ".Rds", sep=""))
}
