...
Who are the top publishers of games on BGG and how do they relate to community ratings?

# Publishers

## Top Publishers by BGG Outcome

Which publishers have published the most games on BGG? Which publishers are the highest rated? Most popular? 

'Uncredited' is by far the most common listing for publisher, so I have excluded it from the following visualization.

```{r most common publishers, warning=F, message=F}
...
I'll then plot the distribution of games with the 25 most frequent publishers. What publishers are typically associated with heavier games? What publisher is the highest rated (on average)?

```{r plot publishers}

plot_games_by_categorical(game_publishers,
                          alpha = 0.25,
                          25)

```

Finally, the following (interactive) table displays the mean of each BGG outcome by publisher. Use the filters below the column names to filter through the table by publisher or set a minimum number of games. 

```{r make summary table}

# summarize data
summary_data = game_publishers %>%
...
## Publisher Partial Effects

Which publishers have the largest effect on a game's average?

We could simply look at the games published by each publisher and then take their average rating/average number of user ratings. However, this doesn't account for the fact that some publishers design more complex games than others, or that recently published games tend to have higher averages. 

To get an estimate of each publisher's partial effect on an outcome, I run penalized regressions lasso) of the BGG average/usersrated on dummies for individual game publishers along with the game's weight and effects for year published.
...
$$ Average = \beta_0 + Weight * \beta_1 + Publisher_1 * \beta_2 + Publisher_2 * \beta_3 * Publisher_k ... Publisher_n * \beta_n + YearPublished$$

That is, which publisher has the largest partial effect on a game's average rating conditional on average weight and time?

```{r partial effects of publishers, include=F}
...
seq_average = seq(-.1, .1, .05)
seq_usersrated = seq(0, 0.5, .05)
