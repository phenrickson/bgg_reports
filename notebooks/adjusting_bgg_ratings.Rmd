...
```{r plot distribution of ratings, warning=F, message=F, echo=T, fig.height=5, fig.width=10}
...
```{r look at correlation between rating and weight, warning=F, echo=F}
...
```{r plot games by average and user ratings, fig.height=8, fig.width=10}
...
```{r compare geek vs average, warning=F, message=F, echo=T}
...
```{r flextable for geekratings, warning=F, echo=T}
...
```{r regress rating on weight, warning=F, message=F, echo=T}
...
```{r get the residuals, warning=F, message=F, echo=T}
...
```{r residuals vs complexity, echo=T, warning=F, message=F}
...
```{r adjusted averages, warning=F, message=F, echo=T}
...
complexity_adjusted_ratings %>%
  ...
  head(250) %>%
        ...
        bg(., j = c("geekrating",
                    "adjustedrating"),
           bg = bayesaverage_col_func) %>%
        flextable::align(j = c("published", "game_id",
                               "geekrating", "adjustedrating", "complexity"),
                         align = "center")
...
complexity_adjusted_ratings %>%
        ...
        arrange(desc(adjustedrating)) %>%
        mutate(rankdiff = geekrank-adjustedrank) %>%
  select(published, game_id, name, geekrank, adjustedrank, rankdiff, geekrating, adjustedrating) %>%
        DT::datatable(rownames =F)

```

### Which Games Move Down the Most?
...
complexity_adjusted_ratings %>%
        ...
        select(date, name, geekrating, adjustedrating, rating_diff) %>%
        head(50) %>%
        flextable() %>%
        flextable::autofit() %>%
        bg(j = c("rating_diff"),
           bg = diff_func) %>%
        flextable::align(j = "rating_diff",
                         align = "center")
...
### Movement Within the BGG Top 100
...
complexity_adjusted_ratings %>%
        ...
        mutate(geekrating = bayesaverage,
               ...
               geekrank = rank,
               adjustedrank = adj_rank) %>%
        select(date, name, geekrating, adjustedrating, rank_diff) %>%
...
```{r show the hotness effect on bgg ratings, echo=T}
...
```{r plot average user ratings by year, echo=T}
...
```{r show what this means via the mountain, fig.height=8, fig.width=10, warning=F, message=F}
...
```{r adjust the number of games, warning=F, echo=T} 
...
```{r show what this means via the mountain for the updated rankings, fig.height=8, fig.width=10, warning=F, message=F}
...
```{r interactive data table, warning=F, message=F, echo=F}
...
complexity_adjusted_ratings_out<- complexity_adjusted_ratings %>%
        select(date, game_id, name, adj_bayesaverage)
...
fwrite(adjusted_ratings,
      file = here::here("adjusted_ratings", paste(Sys.Date(),".csv", sep="")))
