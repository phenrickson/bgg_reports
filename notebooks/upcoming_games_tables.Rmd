...
active_games<-DBI::dbGetQuery(bigquerycon, 
                              ...
                              where load_ts = (SELECT MAX(load_ts) as most_recent FROM bgg.api_game_info)') %>%
...
# games number mechanics

# discard what you dont need, said aaron downey!
#rm(game_types)

# get most recent date
most_recent_date = max(as.Date(active_games$load_ts))
...
games_upcoming = games_model %>%
        ...
        filter(usersrated > 5) %>%
        rename(timestamp = load_ts)
#      filter(bayesaverage > 0)

games_template = bgg_outcomes_recipe$template
#names(games_template)[!(names(games_template) %in% names(games_upcoming))]
...
preds_upcoming_tidied = preds_upcoming %>%
        ...
        arrange(desc(bayesaverage_indirect_stan))
...
make_heat_table_func = function(input_preds_table,
        ...
        input_preds_table %>%
                rename(ID = game_id,
                       ...
                       UserRatings = usersrated_stan,
                       Weight = averageweight_stan,
                       Average = average_stan,
                       GeekRating = bayesaverage_indirect_stan) %>%
...
preds_table = preds_upcoming_tidied %>%
        ...
        left_join(., games_upcoming %>%
                  ...
                  by = c("game_id", "name")) %>%
        #    filter(bayesaverage > 0) %>%
        select(game_id, name, yearpublished, contains("stan")) %>%
        arrange(desc(bayesaverage_stan)) %>%
        rename(ID = game_id,
               Name = name,
               Published = yearpublished,
               UserRatings = usersrated_stan,
...
blueRedColorRamp = colorRampPalette(c("dodgerblue", "white", "orange"))
