...
```{r get predictions from training for average, warning=F, message=F, fig.height=8, fig.width=10}
...
```{r get predictions from training for usersrated, warning=F, message=F, fig.height=8, fig.width=10}
...
What games did the models tend to like for the highest average?

```{r look at highest average rating from training resampling, fig.height=8, fig.width=10}
...
```{r look at highest average rating from training resampling, fig.height=20, fig.width=10}
...
plot = ggplotly(
      ...
      #  geom_point(aes(x=.pred ))
        theme_phil()+
        scale_color_colorblind(),
      layout = plotly::layout(yaxis = list(rangeslider = list()))
)
...
all_tuned = bind_rows(average_tuned %>%
                               mutate(outcome = "average"),
                       averageweight_tuned %>%
                               mutate(outcome = "averageweight"),
                       usersrated_tuned %>%
