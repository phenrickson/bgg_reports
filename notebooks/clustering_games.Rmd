---
title: "Finding Similar Boardgames"
output: 
  html_document:
...
This notebook details my approach to clustering board games and finding nearest neighbors for any given game.

For this analysis, I will develop my methods for games games published through `r params$end_training_year`, then apply them to future games.
...
```{r pca loadings for top ten components, fig.height=8}
...
Let's look at the neighbors for a sample of selected games.
 ...
 #       arrange(desc(similarity)) %>%
  ...
  #      rename(`similarity*bayesaverage` = score)  %>%
       mutate(name = factor(name)) %>%
        select(name, neighbor_name, neighbor_average, neighbor_bayesaverage, score) %>%
...
# save
readr::write_rds(pca_mechanics_recipe,
     file = here::here("models", "repository", paste("pca_recipe_", Sys.Date(), ".Rds", sep="")))

readr::write_rds(pca_mechanics_trained,
...
tile_plot(neighbors_cosine %>%
                  filter(neighbor_yearpublished < 2021 | neighbor_name == temp),
          25,
          active_games %>% filter(name == temp) %>% pull(game_id)) +
        ylab("Games")
