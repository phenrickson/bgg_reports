...
find_neighbors_func = function(input_dist, neighbors) {
        apply(input_dist, 1, n_min_func, n = neighbors) %>%
                        rbindlist(idcol = T) %>%
...
neighbors_train = bind_rows(
        ...
        mutate(dist = map(rotation,
                          ~ dist(.x %>%
                                         select(one_of(paste("PC", seq(1, 20), sep=""))) %>%
                                         as.matrix() %>%
                                         as.data.frame(),
                                 method = "manhattan"))) %>%
        mutate(neighbors = map(dist,
