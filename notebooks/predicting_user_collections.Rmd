...
tidy_username = case_when(tidy_username == 'mrbananagrabber' ~ 'Phil', TRUE ~ tidy_username)
...
This notebook contains a set of analyses for analyzing **`r tidy_username`'s** boardgamegeek collection. The bulk of the analysis is focused on building a user-specific predictive model to predict the games that the specified user is likely to own. This enables us to ask questions like, based on the games the user currently owns, what games are a good fit for their collection? What upcoming games are they likely to purchase?
...
if (nrow(foo$frame) > 40) {tree = rpart::prune(foo, cp=.003)} else {tree = foo}
...
owned_glmnet_train_fit %>%
        ...
        ggtitle(paste("What predicts ", tidy_username, "'s collection?", sep=""),
                subtitle = str_wrap(paste("Coefficients from a penalized logistic regression for games owned by specified user. Predictors centered and scaled. Model trained on games published prior to", params$end_training_year), 120))+
...
```{r save locally, eval=T}

suppressWarnings({
suppressMessages({
saveRDS(user_owned_workflow,
     file = here::here("user_workflows",  paste("bgg_user_owned", tidy_username, "workflow.Rds", sep="_")))
})
})
...
```{r rm objects from memory, results = 'hide', include = F}
