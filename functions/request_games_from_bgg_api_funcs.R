...
simple_node_parse = function(node, var) {
        ...
        # faster way
        out = list()
        data = map(node[var], ~ .x %>% xmlToList %>% as.list) %>% rbindlist
...
tidy_bgg_data_xml = function(input_bgg_games_xml_obj,
        ...
        missing_game_ids = game_ids[!(game_ids %in% game_data$game_id)]
...
get_bgg_game_data = function(input_game_ids,
                             batch_size = 400,
                             tidy = F,
                             toJSON = F) {
        ...
        if (length(input_game_ids) < 400) {
               ...
               bgg_games_xml_obj = get_bgg_games_xml(input_game_ids)
                ...
                bgg_game_xml_batches = map(resp_batches, ~ .x[['bgg_games_xml']]) %>% unlist 
                ...
                bgg_games_xml_obj = list("input_game_ids" = input_game_ids_batches,
                                         ...
                                         "bgg_games_xml" = bgg_game_xml_batches)
        ...
        if (tidy == T) {
                ...
                tidy_bgg_game_data = tidy_bgg_data_xml(bgg_games_xml_obj)
                
                out = list("input_game_ids" = tidy_bgg_game_data$input_game_ids,
                           "problem_game_ids" = tidy_bgg_game_data$problem_game_ids,
                           "bgg_games_data" = tidy_bgg_game_data$bgg_game_data)
