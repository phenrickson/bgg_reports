...
Note: the model was trained on games published through 2020, predicting where a game's outcomes would end converge after roughly 2 years of release.
 ...
 #       mutate(estimated = pred_xgbTree) %>%
       # select(game_id, name, yearpublished, average, pred_stan, pred_xgbTree) %>%
        ...
        rename(ID = game_id,
               ...
               Average = average,
               Predicted = pred_xgbTree) %>%
        select(ID, Published, Name, Average, Predicted) %>%
        mutate_if(is.numeric, round, 2) %>%
  #      select(game_id, name, yearpublished, average, pred_stan, pred_xgbTree) %>%
        DT::datatable()
