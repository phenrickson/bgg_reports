...
estimates %>%
        ...
        # select(yearpublished, name, stddev, bayesaverage, est_bayesaverage, diff) %>%
        # arrange(diff) %>%
        # DT::datatable()

# estimates %>%
#         mutate(highlight = case_when(abs(est_bayesaverage-bayesaverage) > 0.01)) %>%
...
estimates %>%
        mutate(decade = plyr::round_any(yearpublished, 5, floor)) %>%
        filter(decade > 1950) %>%
        ...
        scale_fill_gradient(low = 'deepskyblue1',
                            high = 'red')+
        theme_minimal()
...
complexity_adjusted_ratings %>%
        ...
        select(date, name, averageweight, geekrating, adjustedrating, rating_diff) %>%
        head(50) %>%
        flextable() %>%
        flextable::autofit() %>%
        bg(j = c("averageweight"),
           bg = complexity_col_func) %>%
        bg(j = c("geekrating",
                 "adjustedrating"),
           bg = bayesaverage_col_func) %>%
        flextable::align(j = c("averageweight",
                               "geekrating",
                               "adjustedrating",
                               "rating_diff"),
                         align = "center")
...
complexity_adjusted_ratings %>%
        ...
        flextable::autofit()  %>%
                bg(j = c("geekrating",
                 "adjustedrating"),
           bg = bayesaverage_col_func) %>%
        flextable::align(j = c("geekrating",
                               "adjustedrating",
                               "rank_diff"),
                         align = "center")
```
...
readr::write_rds(complexity_model, file = here::here("models", 
                            "complexity_model.Rds"))
