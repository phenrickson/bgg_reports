...
game_and_neighbors = neighbors_cosine %>%
        head(8) %>%
        select(neighbor_name, neighbor_id) %>%
        mutate(name = abbreviate(neighbor_name, 75))

game_add_to_plot = pca_out %>%
        filter(game_id %in% (neighbors_cosine %>% head(8) %>% pull(neighbor_id))) %>%
        mutate(name = abbreviate(name, 75)) %>%
        mutate(name = factor(name,
                             levels = rev(game_and_neighbors$name)))
...
```{r show component loadings, fig.height=12, fig.width=10}
